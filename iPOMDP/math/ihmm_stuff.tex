...
Optimal behavior is ultimately driven by the rewards, which are drawn from the reward distribution $H_R$ for each state-action pair. 
...
\item In POMDP \#1, the reward distribution $R(\cdot|s,a)$ in the last state in the left-turn short arm has a high mean, while all other states have no rewards. In contrast, in POMDP \#2, it is the last state in the right-turn short arm that has a high mean.
\end{itemize}
...
For example, imagine the task of finding a key to unlock a door and get room 1 to room 2. The transition structure within each room corresponds to a template. However, because of the state dimension (key, no key), there are two modules for each room, or four modules in total: (room 1, no key), (room 1, key), (room 2, no key), (room 2, key). The transition from (room 1, key) to (room 2



go as far as you can with these ideas

then consider Sam's example
