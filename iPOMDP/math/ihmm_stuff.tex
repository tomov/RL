...
\begin{figure}
\centering
\includegraphics[scale=1,  trim = 0 20 400 10]{figures/examples.pdf}
\caption{(A) Rooms domain from \cite{Sutton1999}, adapted from \cite{Botvinick2008}. G, goal state, S, starting state. (B) Upper left room with its corresponding option policy (arrows) leading to the subgoal state (SG). Colors indicate option-specific state values.}
\label{fig:examples}
\end{figure}

While the ultimate goal of this project is to design a generative model and inference machinery for a wide range of natural tasks, it is helpful to consider a few illustrative examples of constrained tasks people use to study HRL and actions sequences.

\subsubsection{Machine Learning}

\paragraph{Rooms domain} A popular task used to illustrate HRL is the rooms domain (Figure~\ref{fig:examples}A and B), which consists of a grid world in which some locations (states) are passable while others are not, dividing the space into rooms. Transitions are deterministic and there is often a single goal state in which the task terminates. Usually any transition to the goal state gives some fixed reward, while all other transitions yield zero reward. Alternatively, all states have a negative reward associated with them, with the exception of the goal state which has zero reward. In both cases, the optimal policy is the shortest path from the agent's starting state to the goal state.
...
Another classic task used to study HRI is taxi domain \cite{Dietterich1998}. TODO structure, figure 
...
While the ultimate goal of this project can be seen as discovering a generative distribution 
...
The iPOMDP with subgoals can easily be extended with templates. An example is shown in Figure~\ref{fig:nested}F.
