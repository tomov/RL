...
\item A transition probability vector $T(\cdot | s,a)$ is drawn from a Dirichlet process (DP) \cite{Teh2006} with concentration parameter $\alpha_1$ and base distribution $\bar{T}$.
...
\begin{align*}
\bar{T} \sim \text{GEM}(\alpha_0) \\
T(\cdot | s,a) \sim \text{DP}(\alpha_1, \bar{T}) \\
O(\cdot | s,a) \sim H_O \\
R(\cdot | s,a) \sim H_R \\
